...
export async function userSignup(req, res, next) {
  try {
    ...
    if (existUser) {
      ...
      const createUser = await VendorDb.create(details);
      await Userdb.create({
        name: data.Name,
        email: data.email,
        password: bcrypt.hashSync(password, salt),
...
export async function getVendor(req, res, next) {
  try {
    const getDomainlist = await VendorDb.find();
    res.status(200).json({
...
export async function getList(req, res, next) {
  try {
    const getemployeelist = await VendorDb.find()
...
export async function getoneUser(req, res, next) {
  try {
    ...
    const studentgetoneid = await VendorDb.findById(studentId);
    res.status(200).json({
      message: "get Successfully",
...
export async function updateEmployee(req, res, next) {
  try {
    ...
    const updateList = await VendorDb.findByIdAndUpdate(id, details, {
      new: true,
    });
    res.status(200).json({
...
export async function deleteEmployee(req, res, next) {
  try {
    ...
    const employeeDelete = await VendorDb.findByIdAndDelete(employeeId);
    res.status(200).json({
