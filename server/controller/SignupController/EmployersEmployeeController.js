import EmployersEmployeeSignUpDb from "../../model/signupModel/EmployersEnployeeModel.js";
...
export async function userSignup(req, res, next) {
  ...
  try {
    ...
    if (existUser) {
      ...
      const createUser = await EmployersEmployeeSignUpDb.create(details);
      await Userdb.create({
        name: data.LastName,
        email: data.email,
        password: bcrypt.hashSync(password, salt),
...
export async function getStudent(req, res, next) {
  try {
    const getDomainlist = await EmployersEmployeeSignUpDb.find();
    res.status(200).json({
...
export async function getList(req, res, next) {
  try {
    const getemployeelist = await EmployersEmployeeSignUpDb.find();
...
export async function getoneUser(req, res, next) {
  try {
    ...
    const studentgetoneid = await EmployersEmployeeSignUpDb.findById(studentId);
    res.status(200).json({
      message: "get Successfully",
...
export async function updateEmployee(req, res, next) {
  try {
    ...
    const updateList = await EmployersEmployeeSignUpDb.findByIdAndUpdate(
      id,
      details,
      {
        new: true,
...
export async function deleteEmployee(req, res, next) {
  try {
    ...
    const employeeDelete = await EmployersEmployeeSignUpDb.findByIdAndDelete(
      employeeId
    );
    res.status(200).json({
