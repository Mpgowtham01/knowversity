import backgroundDB from "../../model/signupModel/BackgroundVerificationModel.js";
...
export async function userSignup(req, res, next) {
  try {
    ...
    if (existUser) {
      ...
      const createUser = await backgroundDB.create(details);
...
export async function getList(req, res, next) {
  try {
    const getemployeelist = await backgroundDB.find();
...
export async function updateEmployee(req, res, next) {
  try {
    ...
    const updateList = await backgroundDB.findByIdAndUpdate(id, details, {
      new: true,
    });
    res.status(200).json({
...
export async function deleteEmployee(req, res, next) {
  try {
    ...
    const employeeDelete = await backgroundDB.findByIdAndDelete(employeeId);
    res.status(200).json({
...
export async function BackRoundVerificationLogin(req, res, next) {
  try {
    const data = req.body;
    const existUser = await backgroundDB.findOne({
      email: data.email,
      role: data.role,
    });
    if (existUser) {
